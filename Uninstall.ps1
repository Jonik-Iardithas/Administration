...
$Global:StartMenu = "$env:APPDATA\Microsoft\Windows\Start Menu\Programs\PowerShellTools\Administration"

$Txt_List = @{
    Form      = "Uninstaller für Administration"
    Label0    = "Dieser Uninstaller entfernt das Programm Administration und seine Komponenten."
    ...
    {Add_Click(
        {
            ...
            If (Get-ScheduledTask -TaskName 'Administration' -ErrorAction SilentlyContinue)
                {
                    Unregister-ScheduledTask -TaskName 'Administration' -AsJob | Out-Null
                }
