...
$Global:Settings = "$env:LOCALAPPDATA\PowerShellTools\Administration\Settings.ini"
$Global:AppDir = "$env:LOCALAPPDATA\PowerShellTools\Administration"
$Global:Archive = "$PSScriptRoot\Administration.zip"
$Global:Icons = "$PSScriptRoot\Administration - Icons_Custom.zip"
$Global:Path = "$env:ProgramFiles\PowerShellTools\Administration"
$Global:Desktop = "$env:USERPROFILE\Desktop\Administration.lnk"
$Global:StartMenu = "$env:APPDATA\Microsoft\Windows\Start Menu\Programs\PowerShellTools\Administration\Administration.lnk"
$Global:Uninstall = "$env:APPDATA\Microsoft\Windows\Start Menu\Programs\PowerShellTools\Administration\Uninstall (Administration).lnk"
$Global:Content = @("IconFolder = $Global:Path\Icons\")

$Txt_List = @{
    Form       = "Installer für Administration"
    LabelA0    = "Dieser Installer bereitet das Programm Administration für die Installation vor."
                ...
                If ($clb_Box.GetItemCheckState(0) -eq [System.Windows.Forms.CheckState]::Checked)
                    {
                        ...
                        $Shortcut.Arguments = "-ExecutionPolicy Bypass -WindowStyle Hidden -File `"$Global:Path\Administration.ps1`""
                        $Shortcut.WorkingDirectory = $Global:Path
                        $Shortcut.IconLocation = "$Global:Path\Icons\Administration.ico"
                        $Shortcut.Save()
